...
2. **`NEXT_PUBLIC_` prefix** means the variable is inlined into the client bundle at build time and visible to anyone. Only use it for values that are safe to expose publicly (e.g. brand slug, Sentry DSN). Never use it for API keys, secrets, or tokens. **Do NOT add `NEXT_PUBLIC_*` vars to Vercel project settings** - they are public, so they belong in each app's ``.env.production` in each app, not in Vercel's secret store.

3. **Server-only variables** (no `NEXT_PUBLIC_` prefix) are only available in server components, server actions, route handlers, and `next.config.ts`. Use this for all secrets (`API_URL`, `BREVO_API_KEY`, `PACMS_API_KEY`, `SENTRY_AUTH_TOKEN`). These **must** be set in Vercel project settings.
...
```
API_URL, BREVO_API_KEY, NEXT_PUBLIC_SENTRY_DSN, PACMS_API_KEY, SENTRY_AUTH_TOKEN, SENTRY_ORG, SENTRY_PROJECT
```
