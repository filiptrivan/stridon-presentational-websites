...
**Known gap vs pa-storefront: nothing mechanically enforces this.** There, raw `next/image` is lint-banned and one wrapper owns the branch, so no call site can forget. Here there are 16 raw `next/image` sites (mostly local assets, which need no guard) and no wrapper, so a new PACMS-image surface must remember on its own — check the field's nullability in `packages/shared/src/types/api.ts` and branch. A wrapper is not obviously worth it at this size; revisit if a third PACMS-image card appears.
